...
<div class="form-group has-error has-feedback" id="${fieldId}" data-z="${z}"
  ...
  <div class="col-md-6">
    <input id="${fieldName}" name="${fieldName}" data-th-value="*{{${fieldName}}}" type="text" class="form-control datetimepicker"
      ...
      data-toggle="tooltip" <#if required == true></#if> /> <span
...
<script type="text/javascript">
	if(typeof window.orientation !== 'undefined') {
		 	document.getElementById("${fieldName}").classList.remove('datetimepicker');
		 	document.getElementById("${fieldName}").classList.add("datetime-local");
	} 
</script>
</#macro>
